...
def generation_response(
    status="ANSWERED", answer="근거 기반 답변", citations=None, refusal=None
):
    if citations is None:
        citations = [5001]
    message = SimpleNamespace(
        content=json.dumps(
            {"status": status, "answer": answer, "citations": citations},
            ensure_ascii=False,
        ),
        refusal=refusal,
    )
...
def test_generate_parses_structured_response(citations):
    ...
    status, answer, actual_citations = GenerationService(client).generate(
        "질문", evidence()
    )
...
def test_generate_preserves_question_language(question, answer):
    ...
    _, actual_answer, _ = GenerationService(client).generate(question, evidence())
...
@pytest.mark.parametrize(
    "payload",
    [
        {"answer": "답변", "citations": [5001]},
        {"status": "UNKNOWN", "answer": "답변", "citations": [5001]},
        {
            "status": "ANSWERED",
            "answer": "답변",
            "citations": [5001],
...
def test_generate_rejects_schema_invalid_structured_output(payload):
    client = Mock()
...
def test_generate_normalizes_insufficient_evidence_response():
    client = Mock()
    client.chat.completions.create.return_value = generation_response(
        status="INSUFFICIENT_EVIDENCE",
        answer="모델이 반환한 불필요한 답변",
        citations=[5001],
    )

    result = GenerationService(client).generate("계약직도 신청 가능한가요?", evidence())

    assert result == ("INSUFFICIENT_EVIDENCE", None, [])


def test_generate_returns_answered_for_stale_premise_correction():
    client = Mock()
    client.chat.completions.create.return_value = generation_response(
        answer="지원 금액은 73,000포인트가 아니라 82,000포인트입니다.",
        citations=[5001],
    )
    stale_evidence = [
        {"chunk_id": 5001, "content": "현재 지원 금액은 82,000포인트입니다."}
    ]

    result = GenerationService(client).generate(
        "지원 금액이 73,000포인트인가요?", stale_evidence
    )

    assert result == (
        "ANSWERED",
        "지원 금액은 73,000포인트가 아니라 82,000포인트입니다.",
        [5001],
    )


@pytest.mark.parametrize(
    ("question", "evidence_content"),
    [
        ("계약직도 신청 가능한가요?", "정규직은 신청할 수 있습니다."),
        ("직원이 스스로 승인할 수 있나요?", "팀 리더가 승인합니다."),
        ("공휴일에도 오전 9시에 운영하나요?", "평일 오전 9시에 운영합니다."),
    ],
    ids=["unsupported-eligibility", "unsupported-actor", "unsupported-condition-timing"],
)
def test_generate_returns_insufficient_when_requested_dimension_is_unsupported(
    question, evidence_content
):
    client = Mock()
    client.chat.completions.create.return_value = generation_response(
    ...
    result = GenerationService(client).generate(
        question, [{"chunk_id": 5001, "content": evidence_content}]
    )
...
def test_generate_prompt_contains_all_five_evidence_items():
    client = Mock()
    client.chat.completions.create.return_value = generation_response()
    five_items = [
        {"chunk_id": chunk_id, "content": f"근거 {chunk_id}"}
        for chunk_id in range(5001, 5006)
    ]

    GenerationService(client).generate("질문", five_items)
...
def test_generate_passes_selected_model_and_generation_parameters():
    ...
    assert set(schema["properties"]) == {"status", "answer", "citations"}
    assert schema["required"] == ["status", "answer", "citations"]
